...
const checkValue = DEBUG ? require("assert").strictEqual : () => {};
...
class Parser {
  ...
  constructorFn = PlainConstructor;
  fixedSize = 0;
  ...
  stream() {
    ...
    const stream = new TransformStream({
      ...
      flush(done) {
        if (currentCallback) {
          currentCallback(
            Buffer.concat(chunks, chunkTotalLength),
            offset,
            true
          );
        }
  ...
  choice(varName, options, getChoice) {
    ...
    this._addReadFunc((read, obj, cb) => {
      const choice = getChoice(obj);
      const TypeConstructor = choice.constructorFn;
      choice.readFunc(read, new TypeConstructor(obj), (read, inner, done) => {
        writeFunc(obj, inner);
        cb(read, obj, done);
      });
    });
  ...
  array(varName, options, type) {
    let writeFunc = getWriteFunc(varName, options);
    ...
    this._addReadFunc((read, obj, cb) => {
      ...
      function readNext() {
        typeRead(read, new TypeConstructor(obj), (read, val, done) => {
  ...
  string(varName, options) {
    ...
    if (options.zeroTerminated) {
      ...
      this._addReadLength(
        writeFunc,
        length,
        (buf, offset, len) => buf.toString(encoding, offset, offset + len),
        true
      );
    } else {
  ...
  buffer(varName, options) {
    ...
    if (options && options.clone) {
      ...
      writeFunc = function copyBuf(obj, val) {
        const buf = Buffer.alloc(val.length);
        val.copy(buf);
        nextFunc(obj, buf);
      };
    }
    ...
    this._addReadLength(
      writeFunc,
      length,
      (buf, offset, len) => buf.slice(offset, offset + len),
      true
    );
  }
  ...
  fixedSizeNest(varName, options) {
    ...
    this._addReadFunc(function fixedSizeNest(read, obj, cb) {
      ...
      function readFixed(reqLength, cb) {
        if (len < reqLength) {
          return read(len, (buffer, offset, done) => {
            realEnd = done;
      ...
      typeRead(readFixed, new TypeConstructor(obj), (_read, val, done) => {
        writeFunc(obj, val);

        if (realEnd && len > 0) {
          read(len, (_buffer, _offset, done) => {
            cb(read, obj, done);
          });
        } else {
  ...
  processBitfield(bitfield, length) {
    let sum = length;
    ...
    const readBitfield = bitfield.reduceRight(
      (nextFunc, req) => {
        ...
        let processFunc = (read, obj, remainder, cb) => {
        ...
        for (let requested = 0; requested < requiredBytes; requested++) {
  ...
  _addReadLength(writeFunc, length, readVal, allowLess) {
    length = wrapLength(length);
